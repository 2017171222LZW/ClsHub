...
from models.model_factory import GoogleFactory as ModelFactory
...
@register_model
def mygooglenet(*, pretrained=False, progress: bool = True, num_classes=1000, **kwargs: Any) -> ModelFactory:
    if pretrained:
        weights = GoogLeNet_Weights.verify(GoogLeNet_Weights.IMAGENET1K_V1)
    else:
        weights = None
    ...
    model = ModelFactory(model, num_classes=num_classes)
    return model
...
if __name__ == '__main__':
    ...
    model = mygooglenet(pretrained=False, num_classes=200)
    model.eval()
    out = model(data)
    print(out.shape)
