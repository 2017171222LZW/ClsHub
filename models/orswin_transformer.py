...
@register_model
def swin_t(*, pretrained=False, progress: bool = True, **kwargs: Any) -> SwinTransformer:
    if pretrained:
        weights = Swin_T_Weights.verify(Swin_T_Weights.IMAGENET1K_V1)
    else:
        weights = Swin_T_Weights.verify(None)
...
@register_model
def swin_s(*, pretrained=False, progress: bool = True, **kwargs: Any) -> SwinTransformer:
    if pretrained:
        weights = Swin_S_Weights.verify(Swin_S_Weights.IMAGENET1K_V1)
    else:
        weights = Swin_S_Weights.verify(None)
...
@register_model
def swin_b(*, pretrained=False, progress: bool = True, **kwargs: Any) -> SwinTransformer:
    if pretrained:
        weights = Swin_B_Weights.verify(Swin_B_Weights.IMAGENET1K_V1)
    else:
        weights = Swin_B_Weights.verify(None)
...
if __name__ == '__main__':
    ...
    model = swin_t(pretrained=True, num_classes=100)
    out = model(data)
    print(out.shape)
