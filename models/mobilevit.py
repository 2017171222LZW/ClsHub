...
from models.mobilevit_module import InvertedResidual, MobileVitBlock
...
@register_model
def MobileViT_XXS(img_size = 256, num_classes = 1000, **kwargs):
...
@register_model
def MobileViT_XS(img_size = 256, num_classes = 1000, **kwargs):
    cfg_xs = model_cfg["xs"]
    model_xs = MobileViT(img_size, cfg_xs["features"], cfg_xs["d"], cfg_xs["layers"], cfg_xs["expansion_ratio"], num_classes)
    return model_xs

@register_model
def MobileViT_S(img_size = 256, num_classes = 1000, **kwargs):
    cfg_s = model_cfg["s"]
