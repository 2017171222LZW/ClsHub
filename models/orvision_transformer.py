...
from typing import Any, Callable, List, NamedTuple, Dict
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        q_m = 1.0 / torch.sqrt(torch.sum(torch.pow(x, 2), dim=-1)).unsqueeze(-1)
        qk = x * q_m

        x, _ = self.self_attention(query=qk, key=qk, value=x, need_weights=False)
        x = self.dropout(x)
        x = x + input
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        dropout: float,
        attention_dropout: float,
        norm_layer: Callable[..., torch.nn.Module] = nn.Identity,
    ):
        super().__init__()
...
@register_model
def myvit_b_16(*, pretrained=True, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    if pretrained:
...
@register_model
def myvit_b_32(*, pretrained=False, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    if pretrained:
...
@register_model
def myvit_l_16(*, pretrained=False, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    if pretrained:
...
@register_model
def myvit_l_32(*, pretrained=False, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    if pretrained:
...
@register_model
def myvit_h_14(*, pretrained=False, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    if pretrained:
...
if __name__ == '__main__':
    ...
    # model = myvit_b_16(pretrained=False, num_classes=100)
    # out = model(data)
    # print(out.shape)
    ...
    print('flops:', flops)
    print('params:', params)
    # flops: 11271224992.0     11.27GFlops
    # params: 58128876.0